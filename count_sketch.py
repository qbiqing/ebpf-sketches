#!/usr/bin/python
#
# univmon.py eBPF CountSketch implementation
#
...
def queryLayer(pkt, cm_value):
    def read(i):
        ...
        # print(f"read row {i} and column {idx}")

        return cm_value.values[i][idx]*pn
    return np.median([read(i) for i in range(CS_ROWS)])
...
def query_sketch(cmd):
    ...
    cs_table = b.get_table("countsketch")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='eBPF Count Sketch implementation')
